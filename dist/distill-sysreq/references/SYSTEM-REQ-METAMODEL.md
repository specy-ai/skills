...
When the product requirements that originated the system requirements are stored in a file (a `.prd` file as defined in PRODUCT-REQ-METAMODEL.md), the requirement set MUST declare a **prd-source** attribute containing the relative path to that file. This makes the provenance chain file-resolvable: an agent or tool can follow `prd-source` to read the PRD, then match each requirement's `source` field to the corresponding PRD element (feature, user story, acceptance criterion, constraint, or goal). Combined with the `requirements-source` attribute on the domain model (defined in DOMAIN-METAMODEL.md), this creates a complete file-level traceability chain:
