...
export class SpecyDomainValidator {
     ...
     */
    checkEscalationChainTerminates(chain, accept) {
        const last = chain.steps.at(-1);
        if (!last?.action)
            return;
        const kind = last.action.kind;
        if (kind === 'retry' || kind === 'compensate') {
            accept('error', `An escalation chain must terminate, but its last step "${last.name}" is a "${kind}", which can ` +
                'itself fail. End the chain with alert, suspend, or manual.', { node: last, property: 'name' });
        }
    }
    /**
     ...
     */
    checkInterfaceRoleConsistency(iface, accept) {
        ...
        if (iface.role === 'spi') {
            if (describes.length === 0) {
                accept('warning', 'An SPI is a driven port: it should name the infrastructure service or repository that fulfils ' +
                    'its contract, with `describes <Provider>`.', { node: iface, property: 'name' });
            }
            if (exposes.length > 0) {
                accept('error', '`exposes` selects operations that already exist, which belongs to an API (a driving port). ' +
                    'An SPI defines its contract instead — declare the operation signatures directly.', { node: iface, property: 'name' });
            }
        }
        else if (iface.role === 'api' && describes.length > 0) {
            accept('error', '`describes` names the provider of a driven port, which belongs to an SPI. An API publishes ' +
                'operations its module already owns — use `exposes`.', { node: iface, property: 'name' });
        }
    }
    /**
     ...
     */
    checkProjectedByRequiresProjection(entity, accept) {
        if (entity.projectedBy && entity.syncedVia?.pattern !== 'asynchronous-projection') {
            accept('error', '`projected-by` names the adapter maintaining a local read-model, so it is only meaningful with ' +
                '`synced-via asynchronous-projection`. A synchronous lookup keeps no local copy.', { node: entity, property: 'name' });
        }
    }
    /**
     ...
     */
    checkEventAboutEntityCarriesItsIdentifier(event, accept) {
        if (!event.about)
            return;
        ...
        if (!carriesId) {
            accept('warning', `Event "${event.name}" is about ${entity} but carries no field referencing its identifier ` +
                `(expected something like \`${expected}\`). The fact cannot be correlated back to the ` +
                `${entity} it happened to.`, { node: event, property: 'name' });
        }
    }
    /**
     ...
     */
    checkValueTypedEnumHasCodeField(enumDef, accept) {
        if (!enumDef.valueType)
            return;
        const valueType = findValueDef(enumDef, enumDef.valueType);
        if (!valueType)
            return; // unresolved type — not this check's business
        const codeFields = valueType.fields.fields.filter(f => f.constraints.some(c => c.kind === 'code'));
        if (codeFields.length !== 1) {
            accept(codeFields.length === 0 ? 'error' : 'warning', `Enum "${enumDef.name}" holds instances of ${enumDef.valueType}, which must declare exactly one ` +
                `\`code\` field to reference them by (found ${codeFields.length}).`, { node: enumDef, property: 'name' });
        }
    }
    /** An aggregate IS its root, so it cannot also list itself as a child entity. */
    checkAggregateIsNotItsOwnChild(aggregate, accept) {
        if (aggregate.entities.entities.includes(aggregate.name)) {
            accept('error', `An aggregate is its own root, so "${aggregate.name}" must not appear in its own \`entities\` ` +
                'list — that block names the non-root children of the cluster.', { node: aggregate, property: 'name' });
        }
    }
    /** An agreement spans distinct aggregates — a truth about one of them is an invariant. */
    checkAgreementParticipantsDiffer(agreement, accept) {
        const distinct = new Set(agreement.participants.participants);
        if (distinct.size < 2) {
            accept('error', `Agreement "${agreement.name}" must span at least two DISTINCT aggregates. A property of a single ` +
                'aggregate is an invariant, which is enforced atomically instead of reconciled.', { node: agreement, property: 'name' });
        }
    }
}
/** Find a ValueDef by name anywhere in the same document. */
function findValueDef(from, name) {
    let root = from;
