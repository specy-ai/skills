...
program
    ...
    .action(async (file: string, options: { format: string; pretty?: boolean; raw?: boolean; strict?: boolean }) => {
        ...
        if (errors.length > 0) {
            ...
            console.error(
                'Emitting best-effort output from the recovered partial AST; unparsed regions are omitted.'
            );
        }
        ...
        try {
            ...
            console.error(`Failed to serialize ${file}: ${(err as Error)?.message ?? String(err)}`);
            process.exit(1);
            return;
        }
