...
---
name: prd-design
description: Input — a product idea, brief, meeting notes, or prose. Output — a `.prd` file. Creates Product Requirement Documents (.prd) following the Specy PRD metamodel. Use this skill whenever the user wants to write a PRD, define a product, capture product requirements, describe features and user stories, define personas, articulate a product vision, plan releases, or structure any product thinking — even if they don't use the word "PRD". Also trigger when the user mentions jobs-to-be-done, user stories, acceptance criteria, product goals, success metrics, or hypotheses in a product context.
user-invocable: true
---
