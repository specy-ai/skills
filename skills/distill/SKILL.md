...
| Class/module named `*Repository`, `*Store`, `*Dao` | The generic type it manages is likely an `entity` (no repository construct — use for resolution patterns) |
...
- Use `sets TypeName { field = value }` blocks; there is no dot-path field targeting syntax (`sets Entity.field to value` does not exist). If you need to update a nested field, resolve the intermediate entity first.
...
Rules for expression bodies in policy and invariant blocks. Expressions appear directly inside the construct body — there are no wrapping `when { }` or `must { }` blocks.
