...
8. **Preconditions guarding operations** → `policy` definitions. Decide placement: file-level `policy` for cross-entity preconditions, entity-scoped inside `policies { }` for entity-specific preconditions. **Properties always true after mutation** → `invariant` definitions inside entity-scoped or value-scoped `invariants { }` blocks.
...
12. Write `.domain.specy` and print summary:
    ```
    ## Extraction — {domain}
...
11. **Test confirmation:** when a test assertion confirms an extracted behaviour (`policy` call, `sets`, `emits`, service call), treat it as validation. When a test asserts a behaviour absent from the extraction, flag it for review — the extraction may be incomplete.
12. Fix obvious errors. For ambiguous cases → `// UNCLEAR`.
13. Write `specy/gaps.report`:
    ```
    # Distill Report — {date}
...
- **Shared types:** duplicate in each `.domain.specy` (each context is self-contained).
