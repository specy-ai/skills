...
`specy-DDD-metamodel.svg` is hand-built. `specy-PRD-metamodel.svg` and `specy-SysReq-metamodel.svg` are generated by `src/diagrams/build.sh` from declarative specs (`src/diagrams/prd_metamodel.py`, `sysreq_metamodel.py`) rendered by `metamodel_diagram.py` in the DDD diagram's style; every node carries a `data-anchor` naming the heading of the matching metamodel section (used by the specy.ai site to make nodes clickable). Edit the specs, not the SVGs.
