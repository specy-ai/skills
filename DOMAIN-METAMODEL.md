...
A boundary within which a model is well-defined and its language is consistent. Words, types, and rules inside a bounded context share a single meaning. A bounded context has a shortname.
...
A unit of decomposition that groups related domain concepts behind a controlled interface. A module encapsulates its internals and exposes only its interfaces; everything else stays hidden.
...
A named surface that exposes a subset of operations from entities or domain services within a module.
...
- 0..n "delegates to" relation with entities and aggregates
- 1..1 "belongs to" relation with a module
...
Infrastructure service are described with an Interface and a set of operations that will be implemented by an adapter of the real infrastructure component.

The Anti-Corruption Layer (ACL) pattern is implemented as an infrastructure service.

Relations:
- 0..n "called by" relation with domain objects (entities, domain services — dependency is strictly one-way)
- 1..1 "belongs to" relation with a module
- 1..1 "has" an interface
...
A policy is a reactive domain rule that listens to internal events and issues commands in response.
A policy is triggered by an internal event, so it listens to one or more internal events.
